...
{
	echo "you can't trigger the intalize script in the browser.";
	exit;
}
...
use Zend\Db\Sql\Ddl\Constraint;
	...
	*/
	...
	$lupdateDatabase = false;

	if(DEVELOPER_MODE)
	{
		$lupdateDatabase = true;
	}
	else
	{
		$sql = new Sql(_adapter,"property");
		$lselect = $sql->Select();
		$lselect->where(array("id"=>"version"));
	...
	if($lupdateDatabase)
	{
		...
		//update property row and insert
		{
			...
			if($lversion == "")
			{
				$sql = new Sql($_adapter,"property");
				$linsert = $sql->Insert();
				$linsert->values(array("id"=>"version","value"=>VERSION));
			...
			$sql = new Sql($_adapter,"property");
			$lupdate =  $sql->update();
			$lupdate->Where(array("id"=>"version"));
			$lupdate->set(array("value"=>VERSION));
			$lresults = $sql->prepareStatementForSqlObject($lupdate)->execute();
		}
	}
